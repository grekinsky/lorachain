...
export class UTXOManager implements IUTXOManager {
  ...
  validateUTXOOwnership(utxo: UTXO, publicKey: string): boolean {
    try {
      ...
      const utxoAddress = this.extractAddressFromLockingScript(
        utxo.lockingScript
      );
  ...
  applyUTXOUpdates(
    ...
    this.logger.debug(
      `Applying batch UTXO updates: ${additions.length} additions, ${removals.length} removals`
    );
  ...
  private extractAddressFromLockingScript(
