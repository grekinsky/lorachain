  ...
  implements ICompressedMeshProtocol
{
  ...
  async sendCompressedMessage(
    ...
    try {
      ...
      if (!dutyCycleOk) {
        this.logger.warn(
          'Duty cycle constraints prevent compression, sending uncompressed',
          {
      ...
      const result = await this.baseMeshProtocol.sendMessage(
        compressedMessage as any
      );
  ...
  async receiveMessage(data: Uint8Array): Promise<MeshMessage | null> {
    try {
      ...
      if (
        !compressedMessage.compression ||
        compressedMessage.compression === CompressionAlgorithm.NONE
      ) {
        return baseMessage;
      }
  ...
  private initializeCompressionSystem(config: CompressedMeshConfig): void {
    ...
    const protobufEngine = factory.createCompressionEngine(
  ...
  private async signMessage(_data?: any): Promise<string> {
