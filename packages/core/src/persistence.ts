...
export class UTXOPersistenceManager {
  ...
  async saveBlock(block: Block): Promise<void> {
    try {
      ...
      this.logger.debug(`Saved block ${block.index} with ${block.transactions.length} transactions`);
    } catch (error) {
  ...
  async saveUTXOTransaction(transaction: UTXOTransaction): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to save UTXO transaction ${transaction.id}: ${error}`);
      throw error;
    }
  }
  ...
  async getUTXOTransaction(id: string): Promise<UTXOTransaction | null> {
    try {
      ...
      return await this.db.get<UTXOTransaction>(txKey, SubLevels.UTXO_TRANSACTIONS);
    } catch (error) {
  ...
  async saveUTXO(utxo: UTXO): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to save UTXO ${utxo.txId}:${utxo.outputIndex}: ${error}`);
      throw error;
    }
  }
  ...
  async deleteUTXO(txId: string, outputIndex: number): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to delete UTXO ${txId}:${outputIndex}: ${error}`);
      throw error;
    }
  }
  ...
  async saveKeyPair(address: string, keyPair: KeyPair): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to save key pair for address ${address}: ${error}`);
      throw error;
    }
  }
  ...
  async getKeyPair(address: string): Promise<KeyPair | null> {
    try {
      const keyPairKey = this.createKeyPairKey(address);
      return await this.db.get<KeyPair>(keyPairKey, SubLevels.CRYPTOGRAPHIC_KEYS);
    } catch (error) {
      this.logger.error(`Failed to get key pair for address ${address}: ${error}`);
      throw error;
    }
  }
  ...
  async saveBlockchainState(state: UTXOBlockchainState): Promise<void> {
    try {
      ...
      this.logger.debug(`Saved blockchain state with ${state.blocks.length} blocks and ${state.utxoSet.size} UTXOs`);
    } catch (error) {
  ...
  async loadBlockchainState(): Promise<UTXOBlockchainState | null> {
    try {
      // Load metadata first
      const latestBlockIndex = await this.db.get<number>('latest_block', SubLevels.METADATA);
      if (latestBlockIndex === null) {
        this.logger.debug('No blockchain state found');
        return null;
      }

      const difficulty = await this.db.get<number>('difficulty', SubLevels.CONFIG) || 2;
      const miningReward = await this.db.get<number>('mining_reward', SubLevels.CONFIG) || 10;
      const utxoRootHash = await this.db.get<string>('utxo_root_hash', SubLevels.METADATA) || '';
      ...
      this.logger.debug(`Loaded blockchain state with ${blocks.length} blocks and ${utxoSet.size} UTXOs`);
      return state;
    } catch (error) {
  ...
  async validateIntegrity(): Promise<ValidationResult> {
    ...
    try {
      ...
      const difficulty = await this.db.get<number>('difficulty', SubLevels.CONFIG);
      if (difficulty === null || difficulty < 1) {
        errors.push('Invalid difficulty configuration');
      }

      this.logger.debug(`Integrity validation completed with ${errors.length} errors`);
  ...
  async repairCorruption(): Promise<RepairResult> {
    ...
    try {
      ...
      this.logger.debug(`Corruption repair completed: ${corruptedBlocks.length} corrupted blocks found`);
