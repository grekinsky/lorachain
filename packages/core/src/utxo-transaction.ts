...
export class UTXOTransactionManager implements IUTXOTransactionManager {
  ...
  createTransaction(
    ...
    this.logger.debug(`Creating UTXO transaction: ${fromAddress} -> ${toAddress}, amount: ${amount}`);
    ...
    if (utxoSelection.totalValue < requiredAmount) {
      throw new Error(`Insufficient funds: need ${requiredAmount}, have ${utxoSelection.totalValue}`);
    }

    // Create transaction inputs
    const inputs: TransactionInput[] = utxoSelection.selectedUTXOs.map((utxo, index) => ({
    ...
    if (outputs.length > 1) {
      ...
      changeOutput.value = utxoSelection.changeAmount - (actualFee - estimatedFee);
    ...
    this.logger.debug(`Created UTXO transaction ${txId} with ${inputs.length} inputs and ${outputs.length} outputs`);
    return transaction;
  }
  ...
  validateTransaction(transaction: UTXOTransaction, utxoManager: IUTXOManager): ValidationResult {
    ...
    for (const input of transaction.inputs) {
      ...
      if (!utxoManager.validateUTXOExists(input.previousTxId, input.outputIndex)) {
        errors.push(`UTXO ${input.previousTxId}:${input.outputIndex} does not exist or is already spent`);
        continue;
      }
      ...
      if (utxo) {
        ...
        if (!input.unlockingScript || input.unlockingScript.length === 0) {
          errors.push(`Input ${input.previousTxId}:${input.outputIndex} missing unlocking script`);
        }
      }
    }
    ...
    for (let i = 0; i < transaction.outputs.length; i++) {
      ...
      if (output.outputIndex !== i) {
        errors.push(`Output ${i} has incorrect output index: ${output.outputIndex}`);
      }
    ...
    if (Math.abs(expectedFee - transaction.fee) > 0.001) {
      errors.push(`Fee mismatch: expected ${expectedFee}, got ${transaction.fee}`);
    }
    ...
    if (inputValue < outputValue) {
      errors.push(`Insufficient input value: inputs=${inputValue}, outputs=${outputValue}`);
    }
  ...
  private signTransaction(
    ...
    for (let i = 0; i < transaction.inputs.length; i++) {
      ...
      const utxo = utxos.find(u => u.txId === input.previousTxId && u.outputIndex === input.outputIndex);
      ...
      const signature = CryptographicService.sign(messageHash, keyPair.privateKey, 'secp256k1');
    ...
    this.logger.debug(`Signed transaction ${transaction.id} with ${transaction.inputs.length} inputs`);
  }

  // Utility methods for transaction analysis
  calculateTransactionValue(transaction: UTXOTransaction): number {
    return transaction.outputs.reduce((total, output) => total + output.value, 0);
  }

  getTransactionInputAddresses(transaction: UTXOTransaction, utxoManager: IUTXOManager): string[] {
