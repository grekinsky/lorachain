...
export interface ICompressedMeshProtocol {
  ...
  requestDictionary(
    dictionaryId: string,
    fromNode: string
  ): Promise<CompressionDictionary | null>;
  ...
  // Events
  on(
    event: 'compression_stats_updated',
    listener: (stats: CompressionStats) => void
  ): void;
  on(
    event: 'compression_error',
    listener: (error: CompressionError) => void
  ): void;
  on(
    event: 'dictionary_updated',
    listener: (dictionaryId: string) => void
  ): void;
  on(
    event: 'duty_cycle_optimized',
    listener: (efficiency: DutyCycleEfficiency) => void
  ): void;
}
...
export interface IAdaptiveCompressionSelector {
  ...
  updateAlgorithmPreferences(
    preferences: Map<MessageType, CompressionAlgorithm[]>
  ): void;
  ...
  getDutyCycleOptimizedAlgorithm(
...
export interface ICompressionEventEmitter {
  on(
    ...
    listener: (result: CompressionResult) => void
  ): void;
  on(
    event: 'decompression_completed',
    listener: (result: DecompressionResult) => void
  ): void;
  on(
    event: 'compression_error',
    listener: (error: CompressionError) => void
  ...
  on(
    ...
    listener: (data: AlgorithmSelectionData) => void
  ): void;
  on(
    event: 'dictionary_created',
    listener: (dictionary: CompressionDictionary) => void
  ): void;
  on(
    event: 'dictionary_updated',
    listener: (dictionaryId: string, changes: DictionaryChanges) => void
  ): void;
