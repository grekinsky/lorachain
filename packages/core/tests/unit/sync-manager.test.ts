...
describe('UTXOSyncManager', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should initialize with custom configuration', () => {
      ...
      expect(customSyncManager.getSyncProgress().state).toBe(
        UTXOSyncState.DISCOVERING
      );
    });
  ...
  describe('Sync Lifecycle', () => {
    test('should prevent multiple concurrent sync operations', async () => {
      ...
      vi.spyOn(syncManager as any, 'discoverPeers').mockResolvedValue(
        undefined
      );
      vi.spyOn(syncManager as any, 'negotiateCapabilities').mockResolvedValue(
        undefined
      );
      vi.spyOn(syncManager as any, 'syncHeaders').mockResolvedValue(undefined);
      vi.spyOn(syncManager as any, 'syncUTXOSetInternal').mockResolvedValue(
        undefined
      );
    ...
    test('should handle sync errors gracefully', async () => {
      ...
      vi.spyOn(syncManager as any, 'discoverPeers').mockRejectedValue(
  ...
  describe('UTXO Header Synchronization', () => {
    ...
    test('should validate header chain integrity', async () => {
      ...
      await expect(syncManager.syncUTXOHeaders(1, 2)).rejects.toThrow(
  ...
  describe('UTXO Block Synchronization', () => {
    test('should sync blocks with different strategies', async () => {
      ...
      vi.spyOn(syncManager as any, 'detectNetworkType').mockResolvedValue(
        'internet'
      );
  ...
  describe('Mempool Synchronization', () => {
    test('should sync pending transactions', async () => {
      ...
      vi.spyOn(syncManager as any, 'getLocalTransactionIds').mockResolvedValue(
        localTxIds
      );
      vi.spyOn(syncManager as any, 'getRemoteTransactionIds').mockResolvedValue(
        remoteTxIds
      );
      vi.spyOn(
        syncManager as any,
        'downloadTransactionBatch'
      ).mockResolvedValue(missingTxs);
      vi.spyOn(syncManager as any, 'validateTransaction').mockResolvedValue(
        true
      );
  ...
  describe('Error Handling', () => {
    test('should handle peer discovery timeout', async () => {
      ...
      vi.spyOn(syncManager as any, 'discoverPeers').mockRejectedValue(
        new Error('Peer discovery timeout')
      );
    ...
    test('should handle validation errors gracefully', async () => {
      ...
      await expect(syncManager.syncUTXOHeaders(1, 1)).rejects.toThrow(
        'Invalid header chain at height 1: Invalid hash'
      );
    });
  });
